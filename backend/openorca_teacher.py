...
class OpenOrcaTeacher:
    def __init__(self, model_name="TheBloke/Mistral-7B-OpenOrca-GGUF", model_file="mistral-7b-openorca.Q4_K_M.gguf", model_type="mistral", gpu_layers=500, context_length=2048):
        config = Config(context_length=2048, gpu_layers=50)
        auto_config = AutoConfig(config=config, model_type="mistral")
        self.llm = AutoModelForCausalLM.from_pretrained(model_name, model_file=model_file, model_type=model_type, config=auto_config, gpu_layers=gpu_layers)
        self.instructions = """
        Role and Goal: As 'Anki English Explanations', I am specialized in providing clear, insightful explanations of English words in specific contexts. My primary function is to enhance the user's understanding of English vocabulary. For each word, I offer two types of explanations: a 'Direct Explanation' for a detailed, educational understanding, and a 'Simple Analogy' for an easy, relatable comprehension.
        ...
        Obs: I don't need to repeat the phrase received, please just send directly the explanation of it in the format "Direct Explanation" and the "Simple Analogy".
        """

    def explain(self, word, context):
